...
export default function Settings() {
    ...
    // Resizable canvas width state (persisted in localStorage)
    // Default 550px fits a 480px image with optimal padding and positions splitter towards the right
    const [canvasWidth, setCanvasWidth] = useState(() => {
        try {
            localStorage.removeItem('settings_sidebar_width');
            localStorage.removeItem('settings_sidebar_width_v2');
        } catch { }
        const saved = localStorage.getItem('settings_canvas_width_v2');
        const parsed = saved ? Number.parseInt(saved, 10) : Number.NaN;
        if (!Number.isNaN(parsed) && parsed >= 420 && parsed <= 1400) return parsed;
    ...
    // Canvas image max-width control (persisted in localStorage)
    const [imageMaxWidth, setImageMaxWidth] = useState(() => {
        const saved = localStorage.getItem('settings_canvas_image_size');
        return (saved === '100%' || saved === '480px') ? saved : '480px';
    });
    ...
    const resetSidebarToOptimal = () => {
        setCanvasWidth(DEFAULT_CANVAS_WIDTH);
        localStorage.setItem('settings_canvas_width_v2', String(DEFAULT_CANVAS_WIDTH));
    };

    useEffect(() => {
        const handleMouseMove = (e) => {
            if (!isResizingSidebarRef.current || !mainContainerRef.current) return;
            const containerRect = mainContainerRef.current.getBoundingClientRect();
            // Since canvas is the right panel, canvas width = containerRect.right - e.clientX
            let newWidth = containerRect.right - e.clientX;
            const minCanvasW = 420;
            const maxCanvasW = Math.max(minCanvasW, containerRect.width - 280); // leave 280px min for left sidebar
            newWidth = Math.max(minCanvasW, Math.min(newWidth, maxCanvasW));
        ...
        const handleMouseUp = () => {
            if (isResizingSidebarRef.current) {
                ...
                setCanvasWidth(current => {
                    localStorage.setItem('settings_canvas_width_v2', String(Math.round(current)));
                    return current;
                });
            }
        };
    ...
    // 2D Grid Drag & Drop for Page Preview Modal
    ...
    const handleGridDragStart = (e, index) => {
        ...
        // Defer state update so browser captures native drag image without aborting
        requestAnimationFrame(() => {
            setDraggedPageIndex(index);
        });
    };

    const handleGridDragOver = (e, index) => {
        e.preventDefault();
    ...
    const handleGridDragLeave = (e, index) => {
    ...
    const handleGridDrop = async (e, targetIndex) => {
        e.preventDefault();
        setDragOverPageIndex(null);
        const sourceIndex = draggedPageIndex;
        ...
        try {
            await client.put('/api/v1/pages/reorder', { ids: items.map(p => p.id) });
        } catch (err) {
    ...
    const handleDropToFolder = async (e, targetFolderId) => {
        ...
        if (flowIdStr) {
            ...
            const draggedFolderId = Number.parseInt(folderIdStr, 10);
            if (draggedFolderId === targetFolderId) return;

            // Optimistic UI
            setFolders(prev => prev.map(f => f.id === draggedFolderId ? { ...f, parent_id: targetFolderId } : f));
            try {
                const payload = targetFolderId === null ? { move_to_root: true } : { parent_id: targetFolderId };
                await client.put(`/api/v1/folders/${draggedFolderId}`, payload);
            } catch (err) {
                fetchFoldersAndFlows();
                Swal.fire('Error', err.response?.data?.detail || 'Failed to move folder', 'error');
            }
        }
    };
    ...
    const handleAddPage = async () => {
        ...
        const { value: formValues } = await Swal.fire({
            ...
            didOpen: () => {
                ...
                const nameInput = document.getElementById('swal-page-name');
                if (fileInput && nameInput) {
                    fileInput.addEventListener('change', (e) => {
                        ...
                        if (file) {
                            ...
                            // If user hasn't typed anything or if it equals previous filename, auto-fill
    ...
    const handleRemasterFolderUpload = async (e) => {
        const rawFiles = Array.from(e.target.files).filter(f => f.type.startsWith('image/'));
        e.target.value = '';
        if (rawFiles.length === 0) {
            return Swal.fire('Warning', 'No image files found in the selected folder.', 'warning');
        }

        if (!selectedFlow) {
            return Swal.fire('Error', 'Please select a flow first.', 'error');
        }
        ...
        files.forEach(file => {
            const baseName = (file.name.substring(0, file.name.lastIndexOf('.')) || file.name).toLowerCase().trim();
            const existing = pages.find(p => p.page_name.toLowerCase().trim() === baseName);
            if (existing) {
                matchingPages.push({ file, page: existing });
            } else {
                newFiles.push(file);
            }
        });

        let mode = 'MATCH_AND_ADD';

        if (pages.length === 0) {
            const confirmResult = await Swal.fire({
                title: 'Upload Master Folder',
                text: `Upload ${files.length} images as master baseline for "${selectedFlow.name}"?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#7c3aed',
                confirmButtonText: 'Upload Master'
            });
            if (!confirmResult.isConfirmed) return;
            mode = 'ADD_ALL';
        } else if (matchingPages.length > 0) {
            const matchInfo = matchingPages.length === pages.length
                ? `All ${matchingPages.length} existing master pages will be replaced with new images.`
                : `${matchingPages.length} of ${pages.length} existing master pages match by name and will be replaced.` +
                  (newFiles.length > 0 ? ` (${newFiles.length} new pages will be added)` : '');

            const confirmResult = await Swal.fire({
                title: 'Remaster Master Images?',
                html: `
                    <div style="text-align: left; font-size: 14px; line-height: 1.6;">
                        <p style="margin-bottom: 8px;"><strong>Flow:</strong> ${selectedFlow.name}</p>
                        <p style="margin-bottom: 8px;"><strong>Found in folder:</strong> ${files.length} images</p>
                        <p style="color: #6b21a8; background: #f3e8ff; padding: 8px 12px; border-radius: 8px; font-weight: 500;">
                            ✓ ${matchInfo}
                        </p>
                    </div>
                `,
                ...
                showCancelButton: true,
                confirmButtonColor: '#7c3aed',
                confirmButtonText: 'Yes, Remaster Master',
                cancelButtonText: 'Cancel'
            });
            if (!confirmResult.isConfirmed) return;
            mode = 'MATCH_AND_ADD';
            ...
            const confirmResult = await Swal.fire({
                ...
                html: `
                    <div style="text-align: left; font-size: 14px; line-height: 1.6;">
                        <p>No filenames in the folder matched current page names.</p>
                        <p style="margin-top: 8px; color: #b45309; background: #fef3c7; padding: 8px 12px; border-radius: 8px;">
                            Would you like to <strong>replace all ${pages.length} old pages</strong> with the ${files.length} new images in this folder?
                        </p>
                    </div>
                `,
                icon: 'warning',
                ...
                showDenyButton: true,
                confirmButtonColor: '#7c3aed',
                confirmButtonText: 'Replace All Pages',
                denyButtonColor: '#2563eb',
                denyButtonText: 'Match By Order (1 to 1)',
                cancelButtonText: 'Cancel'
            });
        ...
        Swal.fire({
            title: 'Remastering Flow...',
            html: `Preparing upload...`,
            allowOutsideClick: false,
            showCancelButton: true,
            cancelButtonText: 'Cancel',
            showConfirmButton: false,
        ...
        try {
            if (mode === 'WIPE_AND_REPLACE') {
                for (let i = 0; i < pages.length; i++) {
                    if (isCancelled) break;
                    Swal.update({ html: `Removing old pages (${i + 1}/${pages.length})...` });
                    try {
                        await client.delete(`/api/v1/pages/${pages[i].id}`, { signal: controller.signal });
                    } catch (err) {}
                }
                for (let i = 0; i < files.length; i++) {
                    if (isCancelled) break;
                    ...
                    const name = file.name.substring(0, file.name.lastIndexOf('.')) || file.name;
                    const formData = new FormData();
                    formData.append('flow_id', selectedFlow.id);
                    ...
                    try {
                        ...
                        addedCount++;
                        Swal.update({ html: `Uploading new master (${addedCount}/${files.length})...` });
                    } catch (err) {
                        errors.push(`${file.name}: ${err.response?.data?.detail || 'Upload failed'}`);
                    }
                }
            } else if (mode === 'MATCH_BY_ORDER') {
                const total = Math.min(files.length, pages.length);
                for (let i = 0; i < total; i++) {
                    ...
                    formData.append('file', file);
                    try {
                        await client.put(`/api/v1/pages/${page.id}/image`, formData, {
                            headers: { 'Content-Type': 'multipart/form-data' },
                            signal: controller.signal
                        });
                        updatedCount++;
                        Swal.update({ html: `Updating master ${updatedCount}/${total}...` });
                    } catch (err) {
                        errors.push(`${file.name}: ${err.response?.data?.detail || 'Update failed'}`);
                    }
                }
            } else {
                for (let i = 0; i < matchingPages.length; i++) {
                    if (isCancelled) break;
                    const { file, page } = matchingPages[i];
                    const formData = new FormData();
                    formData.append('file', file);
                    try {
                        await client.put(`/api/v1/pages/${page.id}/image`, formData, {
                        ...
                        updatedCount++;
                        Swal.update({ html: `Updating matching pages (${updatedCount}/${matchingPages.length + newFiles.length})...` });
                    } catch (err) {
                        errors.push(`${file.name}: ${err.response?.data?.detail || 'Update failed'}`);
                    }
                }
                ...
                for (let i = 0; i < newFiles.length; i++) {
                    ...
                    try {
                        ...
                        addedCount++;
                        Swal.update({ html: `Uploading new pages (${updatedCount + addedCount}/${matchingPages.length + newFiles.length})...` });
                    } catch (err) {
                        errors.push(`${file.name}: ${err.response?.data?.detail || 'Upload failed'}`);
                    }
                }
        ...
        if (isCancelled) {
            Swal.fire('Cancelled', `Remaster cancelled. Updated ${updatedCount} pages, added ${addedCount} pages.`, 'info');
        } else if (errors.length > 0) {
            Swal.fire({
                icon: 'warning',
                title: 'Remaster Completed with Warnings',
                html: `Updated: ${updatedCount}, Added: ${addedCount}<br/><br/><div className="text-left text-sm text-red-500 overflow-y-auto max-h-32">Errors:<br/>${errors.join('<br/>')}</div>`
            });
        } else {
            Swal.fire({
                icon: 'success',
                title: 'Remaster Successful!',
                text: `Successfully updated ${updatedCount} master image(s)${addedCount > 0 ? ` and added ${addedCount} new page(s)` : ''}.`
            });
        }
    };
    ...
    const handleDeletePage = async (e, page) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                setPages(prev => (Array.isArray(prev) ? prev.filter(p => p.id !== page.id) : []));
    ...
    const handleAutoRedMark = async () => {
        ...
        try {
            ...
            Swal.fire('Error', err.response?.data?.detail || 'Failed to generate auto red mark', 'error');
        }
    };
    ...
    return (
        <>
            <DragDropContext onDragEnd={onDragEndPages}>
                <div ref={mainContainerRef} className="flex h-full relative overflow-hidden">

                    {showSettingsSidebar && (
                        <div
                            id="settings-sidebar"
                            className={clsx(
                                "flex-1 flex flex-col gap-3 sm:gap-4 lg:gap-6 h-full min-w-[280px] overflow-hidden mr-1",
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex-1 flex flex-col min-h-0">
                                <div className="p-4 border-b bg-slate-50 flex justify-between items-center shrink-0">
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        <label htmlFor="remaster-folder-input" className="p-1 hover:bg-slate-200 rounded text-purple-600 cursor-pointer" title="Remaster (Upload Folder to Replace Master Images)">
                                            <RefreshCw size={18} />
                                            <input id="remaster-folder-input" type="file" webkitdirectory="true" multiple className="hidden" onChange={handleRemasterFolderUpload} />
                                        </label>
                    ...
                    {showSettingsSidebar && (
                        <button
                            ...
                            onDoubleClick={resetSidebarToOptimal}
                            aria-label="Canvas resizer"
                            className={clsx(
                                "group relative w-3 -mx-1 flex items-center justify-center cursor-col-resize z-20 shrink-0 select-none bg-transparent border-0 p-0 outline-none",
                                isResizingSidebar ? "cursor-col-resize" : "hover:cursor-col-resize"
                            )}
                            title="Drag to resize | Double-click to fit 480px image"
                        >
                            {/* Vertical divider line */}
                            <div className={clsx(
                                "w-[2px] h-full rounded-full transition-colors duration-150",
                                isResizingSidebar ? "bg-indigo-600" : "bg-slate-200 group-hover:bg-indigo-400"
                            )} />

                            {/* Centered pill handle */}
                            <div className={clsx(
                                "absolute w-4 h-10 rounded-full border shadow-sm flex items-center justify-center transition-all duration-150",
                                isResizingSidebar
                                    ? "bg-indigo-600 border-indigo-700 text-white scale-110 shadow-md"
                                    : "bg-white border-slate-300 text-slate-400 group-hover:border-indigo-400 group-hover:text-indigo-600 group-hover:shadow"
                            )}>
                    ...
                    {/* === RIGHT PANEL: CANVAS === */}
                    <div
                        style={showSettingsSidebar ? { width: `${canvasWidth}px`, maxWidth: 'calc(100% - 280px)' } : { width: '100%' }}
                        className={clsx(
                            "bg-white rounded-xl shadow-sm border border-slate-200 p-3 sm:p-4 flex flex-col h-full overflow-hidden ml-1",
                            showSettingsSidebar ? "shrink-0" : "flex-1"
                        )}
                    >
                        <div className="flex flex-wrap justify-between items-center gap-2 mb-4 pb-4 border-b border-slate-100 shrink-0">
                            <div className="flex gap-2 items-center">
                                <button
                                    type="button"
                                    onClick={() => setShowSettingsSidebar(prev => !prev)}
                                    className="p-2 bg-white border border-slate-200 rounded-lg shadow-sm hover:bg-slate-50 text-slate-600 transition-colors shrink-0"
                                    ...
                                    <div className="flex items-center bg-slate-100 p-1 rounded-lg border border-slate-200 text-xs font-medium text-slate-600">
                                        ...
                                        {[
                                            ...
                                            <button
                                                ...
                                                type="button"
                                                onClick={() => {
                                                    setImageMaxWidth(sz.value);
                                                    localStorage.setItem('settings_canvas_image_size', sz.value);
                                                }}
                                                className={clsx(
                                                    "px-2 py-1 rounded transition-colors",
                                                    imageMaxWidth === sz.value
                                                        ? "bg-white text-indigo-600 shadow-sm font-bold"
                                                        : "text-slate-600 hover:text-slate-900 hover:bg-slate-200/60"
                                                )}
                                                title={`Max width: ${sz.label}`}
                                            >
                                                {sz.label}
                                            </button>
                                        ))}
                        ...
                        <div className="flex-1 overflow-auto bg-slate-50 rounded-lg border border-slate-100 relative flex justify-center items-start p-4">
                            {selectedPage ? (
                                <div style={imageMaxWidth !== '100%' ? { maxWidth: imageMaxWidth, width: '100%' } : { width: '100%' }} className="flex justify-center transition-all duration-200">
                                    <MaskingCanvas
                        ...
                        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50">
                            ...
                            <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 list-none p-0 m-0">
                                {pages.map((page, idx) => (
                                    <li
                                        key={page.id}
                                        aria-label={`Page ${page.page_name}`}
                                        draggable="true"
                                        onDragStart={(e) => handleGridDragStart(e, idx)}
                                        onDragOver={(e) => handleGridDragOver(e, idx)}
                                        onDragLeave={(e) => handleGridDragLeave(e, idx)}
                                        onDrop={(e) => handleGridDrop(e, idx)}
                                        onDragEnd={handleGridDragEnd}
                                        className={clsx(
                                            "bg-white rounded-xl border overflow-hidden flex flex-col cursor-grab active:cursor-grabbing transition-all group relative",
                                            draggedPageIndex === idx
                                                ? "opacity-40 scale-95 border-dashed border-indigo-400 shadow-inner"
                                                : dragOverPageIndex === idx
                                                    ? "ring-4 ring-indigo-500 scale-105 shadow-2xl border-indigo-500 z-20 bg-indigo-50/50"
                                    ...
                                    >
                                        <div className="relative bg-slate-100 flex items-center justify-center p-1 min-h-[120px]">
                                            <img
                                                ...
                                                loading="lazy"
                                            />
                                            <div className="absolute top-2 left-2 bg-indigo-600 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full shadow-md pointer-events-none">
                                                {idx + 1}
                                            </div>

                                            {/* X button on hover (like Run Test) */}
                                            <button
                                                type="button"
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleDeletePage(e, page);
                                                }}
                                                onMouseDown={(e) => e.stopPropagation()}
                                                className={clsx(
                                                    "absolute top-2 right-2 z-10 bg-red-500 hover:bg-red-600 text-white p-1 rounded-full shadow-md transition-opacity duration-200 cursor-pointer",
                                                    draggedPageIndex !== null ? "opacity-0 pointer-events-none" : "opacity-0 group-hover:opacity-100"
                                                )}
