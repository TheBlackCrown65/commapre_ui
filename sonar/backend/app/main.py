...
def get_job_details_real(job_id: str, db: Session = Depends(get_db)):
    ...
    flow_id = db_job.flow_id if db_job else None
    flow_note = db_job.flow.note if db_job and db_job.flow else ""
    status = db_job.status if db_job else "COMPLETED"
    
    if status in ["QUEUED", "PROCESSING"]:
        return {"id": job_id, "status": status, "flow_name": flow_name, "flow_id": flow_id, "flow_note": flow_note, "files": []}

    if os.path.exists(report_path):
        try:
            with open(report_path, "r") as f:
                results = json.load(f)
            return {"id": job_id, "status": status, "flow_name": flow_name, "flow_id": flow_id, "flow_note": flow_note, "results": results}
        except: pass
    ...
    files.sort()
    return {"id": job_id, "status": status, "flow_name": flow_name, "flow_id": flow_id, "flow_note": flow_note, "files": files}
...
def get_jobs_list(department_id: int = None, db: Session = Depends(get_db)):
    ...
    for j in all_jobs:
        ...
        result.append({
            ...
            "flow_name": flow_name,
            "flow_id": db_job.flow_id if db_job else None,
            "flow_note": flow_obj.note if flow_obj else None,
...
def heal_job_baseline(job_id: str, req: HealRequest, request: Request, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    flow_obj = job.flow if job else None
    flow_name = flow_obj.name if flow_obj else "Flow"
    flow_note = flow_obj.note if flow_obj else ""
    return {
        "message": "Healing successful",
        "healed_count": healed_count,
        "flow_id": flow_id,
        "flow_name": flow_name,
        "flow_note": flow_note or ""
    }
